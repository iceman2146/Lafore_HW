// temlist2.cpp
// св€зный список с использованием шаблона
// список, умеющий работать с классом employee
#include <iostream>
using namespace std;

const int LEN = 80;       // ћаксимальна€ длина имен
///////////////////////////////////////////////////////////
class employee            // класс employee
{
private:
	char name[LEN];       // им€ работника
...
{
	cout << "\n  ¬ведите фамилию: "; cin >> e.name;
	cout << "  ¬ведите номер: ";    cin >> e.number;
	return s;
}
//---------------------------------------------------------
...
void linklist<TYPE>::additem(TYPE d)     // добавить данные
{
	...
	first = newlink;                     // теперь перва€ ссылка указывает на эту
}
//---------------------------------------------------------
template<class TYPE>
...
int main()
{
	...
	char ans;                  // ответ пользовател€ ('y' или 'n')

	do
	{
		cin >> emptemp;        // получить данные от пользовател€
		lemp.additem(emptemp); // добавить в emptemp
		cout << "\nѕродолжать (y/n)? ";
		cin >> ans;
	} while(ans != 'n');       // при окончании ввода 
	lemp.display();            // вывести весь св€зный список
	cout << endl;
