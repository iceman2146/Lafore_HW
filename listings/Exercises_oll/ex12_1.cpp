...
int main()
{
	...
	do                                            // данные от пользователя в файл
	{
		...
		file.write((char*)&dist, sizeof(dist));   // записать в файл
		cout << "Продолжать (y/n)?";
		cin >> ch;
	}
	while(ch == 'y');                             // выйти по 'n'
	file.seekg(0);                                // установить указатель на начало файла


	file.read((char*)&dist, sizeof(dist));        // прочитать первое значение
	int count = 0;
	while(!file.eof())                            // выход по EOF
	{
		cout << "\nРасстояние " << ++count << ":";// вывести расстояние
		dist.showdist();
		file.read((char*)&dist, sizeof(dist));    // считать еще расстояние
